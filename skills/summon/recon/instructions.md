...
```json
{
  ...
  "symbols": {
    "src/auth/middleware.ts": [
      { "name": "authenticate", "kind": "function", "line": 5, "end_line": 20, "exported": true, "signature": "(req: Request): boolean" }
    ]
  },
  "dependencies": {
    "src/main.ts": [
      { "target": "src/auth/middleware.ts", "specifiers": ["authenticate"], "resolved": true, "external": false }
    ]
  },
  "graph": {
  ...
  "planning": {
    "primary_entry_points": ["src/main.ts"],
    "dependency_hubs": [{ "path": "src/lib/index.ts", "inbound_dependencies": 8, "export_count": 6, "hotspot_count": 1 }],
    "hotspot_files": [{ "path": "src/auth/middleware.ts", "hotspot_count": 2, "max_complexity": 12 }],
    "priority_files": [{ "path": "src/main.ts", "score": 10.0, "is_entry_point": true, "symbol_count": 3, "complexity": 8 }]
  },
...
- **Incoming edge count:** Files in `planning.dependency_hubs` and `graph.stats.most_imported` are hub files. A hub file and everything that depends on it often form a natural stream boundary.
- **Entry points:** `planning.primary_entry_points` are the curated roots of the dependency tree; `graph.entry_points` remains the raw set. Each primary entry point and its transitive dependency subtree is a strong candidate stream.
...
**Procedure:**
1. For each proposed inter-stream dependency, check whether any file in the downstream stream imports any file in the upstream stream via `dependencies` or `graph.adjacency`
...
Curated entry points (`planning.primary_entry_points`) indicate the application's main execution paths. Streams containing those entry points are foundational and should be scheduled early. Use the full `graph.entry_points` set when you need exhaustive coverage.
...
When budget truncation is active in full mode, recon preserves these sections unconditionally:
...
2. **`planning` object is non-empty** — summon consumes the planning-mode payload, not the full file dump
3. **`dependencies` map exists** — confirms structural graph data made it through
